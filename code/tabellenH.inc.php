...
$_gemeente_taal="d_gemeente_NL";
    ...
    if ($_result->num_rows > 0) {
        while($row = $_result->fetch_assoc()) {  
            ...
            if($_provincieSelectie==2){
                $_cijfer= "<p class=cijferH>".number_format($row["sumTransacties"], 0, '.','.')."</p><p class=cijferTekst>In het Brussels Hoofdstedelijk Gewest zijn er ".number_format($row["sumTransacties"], 0, '.','.')." huizen verkocht.</p>";  
            }
            else
            {
                $_cijfer= "<p class=cijferH>".number_format($row["sumTransacties"], 0, '.','.')."</p><p class=cijferTekst>In provincie $_provincieNaam zijn er ".number_format($row["sumTransacties"], 0, '.','.')." huizen verkocht.</p>";  
            }
        }
    }
    ...
    ROUND(( ((t_prijzen2020.d_prijzenH2020)-(t_prijzen2019.d_prijzenH2019)) / (t_prijzen2019.d_prijzenH2019)*100),2) AS percentage
    FROM
    t_gemeenten
    JOIN t_prijzen2020
    ON t_gemeenten.d_indexGemeenten = t_prijzen2020.d_2020 
    ...
    AND t_prijzen2020.d_prijzenH2020 != 0
    AND t_prijzen2019.d_prijzenH2019 != 0
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) { 
            if ($_provincieSelectie == 2){ 
                ...
                {
                    $_sterksteDaler="<p class='cijfer2'>".number_format($row["percentage"], 2, ',',',')."%</p><p class='tekstPijlen'>In het Brussels Hoofdstedelijk Gewest is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddeld percentage van ".number_format($row["percentage"], 2, ',',',')."% de sterkste daler.";
                }
            }
            else
            {
                ...
                {
                    $_sterksteDaler="<p class='cijfer2'>".number_format($row["percentage"], 2, ',',',')."%</p><p class='tekstPijlen'>In provincie $_provincieNaam is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddeld percentage van ".number_format($row["percentage"], 2, ',',',')."% de sterkste daler.";
                }
             }
         }
    }
    ...
    $_sql = "SELECT
    t_gemeenten.d_gemeente_NL,
    t_prijzen2020.d_prijzenH2020
    FROM
    t_gemeenten
    INNER JOIN t_prijzen2020
    ON t_gemeenten.d_indexGemeenten = t_prijzen2020.d_2020 
    WHERE t_gemeenten.td_indexProvincies= $_provincieSelectie
    order by t_prijzen2020.d_prijzenH2020 DESC
    limit 1";

    $result = $_conn->query($_sql);
    if ($result->num_rows > 0) {  
        while($row = $result->fetch_assoc()) { 
            $_formatPrijzen = number_format($row['d_prijzenH2020'], 0, '.','.'); 
            if ($_provincieSelectie == 2){ 
                $_duursteGemeente="<p class='cijfer1'>".$_formatPrijzen."€</p><p class='tekstPijlen'>In het Brussels Hoofdstedelijk Gewest is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddelde prijs van ".$_formatPrijzen."€ de duurste gemeente.";
            }
            else
            {
                $_duursteGemeente="<p class='cijfer1'>".$_formatPrijzen."€</p><p class='tekstPijlen'>In provincie $_provincieNaam is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddelde prijs van ".$_formatPrijzen."€ de duurste gemeente.";
            }
        }
    }
    ...
    WHERE t_gemeenten.td_indexProvincies= $_provincieSelectie
    AND t_prijzen2020.d_prijzenH2020 != 0
    order by t_prijzen2020.d_prijzenH2020 ASC
    limit 1";
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) { 
            if ($_provincieSelectie == 2){ 
                $_formatPrijzen = number_format($row['d_prijzenH2020'], 0, '.','.'); 
                $_goedkoopsteGemeente="<p class='cijfer2'>".$_formatPrijzen."€</p><p class='tekstPijlen'>In Brussels Hoofdstedelijk Gewest is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddelde prijs van ".$_formatPrijzen."€ de goedkoopste gemeente.";
            }
            else
            {
                $_formatPrijzen = number_format($row['d_prijzenH2020'], 0, '.','.');
                $_goedkoopsteGemeente="<p class='cijfer2'>".$_formatPrijzen."€</p><p class='tekstPijlen'>In provincie $_provincieNaam is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddelde prijs van ".$_formatPrijzen."€ de goedkoopste gemeente."; 
            }
        }
    }
