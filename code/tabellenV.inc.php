...
$_gemeente_taal="d_gemeente_NL";
    ...
    //TABEL PRIJZEN-TRANSACTIES VILLAS
    ...
    if ($_result->num_rows > 0) {
        while($row = $_result->fetch_assoc()) {  
            ...
            if($_provincieSelectie==2){
                $_cijfer= "<p class=cijferV>".number_format($row["sumTransacties"], 0, '.','.')."</p><p class=cijferTekst>In het Brussels Hoofdstedelijk Gewest zijn er ".number_format($row["sumTransacties"], 0, '.','.')." villa's verkocht.</p>";  
            }
            else
            {
                $_cijfer= "<p class=cijferV>".number_format($row["sumTransacties"], 0, '.','.')."</p><p class=cijferTekst>In provincie $_provincieNaam zijn er ".number_format($row["sumTransacties"], 0, '.','.')." villa's verkocht.</p>";  
            }
        }
    }
    ...
    for ($_i=$_arrondissementSelectie; $_i<=$_arrondissementLimiet; $_i++) {
        ...
        if ($result->num_rows > 0) {
            ...
            while($row = $result->fetch_assoc()) {  
                ...
                $_tabelPrijzen.= "<tr><td class='first'>&nbsp;".$row[$_gemeente_taal]."</td><td>".$row[$_transacties2020]."</td><td>".$_formatPrijzen."</td><td  style='$_styleColor1; $_styleColor3;'>".$_berekening1j."</td><td  style='$_styleColor2; $_styleColor3';>".$_berekening10j."</td></tr>";   
    ...
    ROUND(( ((t_prijzen2020.d_prijzenV2020)-(t_prijzen2019.d_prijzenV2019)) / (t_prijzen2019.d_prijzenV2019)*100),2) AS percentage
    FROM
    t_gemeenten
    JOIN t_prijzen2020
    ON t_gemeenten.d_indexGemeenten = t_prijzen2020.d_2020 
    ...
    AND t_prijzen2020.d_prijzenV2020 != 0
    AND t_prijzen2019.d_prijzenV2019 != 0
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) { 
            if ($_provincieSelectie == 2){  
                if($row['percentage']<0){
                    $_sterksteStijger="<p class='tekstPijlen'><br>In het<br>Brussels Hoofdstedelijk Gewest<br>zijn er geen stijgers.</p>";
                }
                else
                {
                    $_sterksteStijger="<p class='cijfer1'>".number_format($row["percentage"], 2, ',',',')."%</p><p class='tekstPijlen'>In het Brussels Hoofdstedelijk Gewest <br>is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddeld percentage van<br>".number_format($row["percentage"], 2, ',',',')."% de sterkste stijger.";
                }
            }
            else
            {
                if($row['percentage']<0){
                    $_sterksteStijger="<p class='tekstPijlen'><br>In de<br>provincie $_provincieNaam<br>zijn er geen stijgers.</p>";
                }
                else
                {
                    $_sterksteStijger="<p class='cijfer1'>".number_format($row["percentage"], 2, ',',',')."%</p><p class='tekstPijlen'>In provincie $_provincieNaam <br>is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddeld percentage van<br>".number_format($row["percentage"], 2, ',',',')."% de sterkste stijger.";
                }
            }
        }
    }
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) { 
            if ($_provincieSelectie == 2){ 
                if($row['percentage']>0){
                    $_sterksteDaler="<p class='tekstPijlen'><br>In het<br>Brussels Hoofdstedelijk Gewest<br>zijn er geen dalers.</p>";
                }
                else
                {
                    $_sterksteDaler="<p class='cijfer2'>".number_format($row["percentage"], 2, ',',',')."%</p><p class='tekstPijlen'>In het Brussels Hoofdstedelijk Gewest <br>is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddeld percentage van<br>".number_format($row["percentage"], 2, ',',',')."% de sterkste daler.";
                }
            }
            else
            {
                if($row['percentage']>0){
                    $_sterksteDaler="<p class='tekstPijlen'><br>In de<br>provincie $_provincieNaam<br>zijn er geen dalers.</p>";
                }
                else
                {
                    $_sterksteDaler="<p class='cijfer2'>".number_format($row["percentage"], 2, ',',',')."%</p><p class='tekstPijlen'>In provincie $_provincieNaam <br>is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddeld percentage van<br>".number_format($row["percentage"], 2, ',',',')."% de sterkste daler.";
                }
             }
         }
    }
    ...
    if ($result->num_rows > 0) {  
        while($row = $result->fetch_assoc()) { 
            $_formatPrijzen = number_format($row['d_prijzenV2020'], 0, '.','.'); 
            if ($_provincieSelectie == 2){ 
                $_duursteGemeente="<p class='cijfer1'>".$_formatPrijzen."€</p><p class='tekstPijlen'>In het<br>Brussels Hoofdstedelijk Gewest <br>is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddelde prijs van<br>".$_formatPrijzen."€ de duurste gemeente.";
            }
            else
            {
                $_duursteGemeente="<p class='cijfer1'>".$_formatPrijzen."€</p><p class='tekstPijlen'>In provincie $_provincieNaam <br>is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddelde prijs van<br>".$_formatPrijzen."€ de duurste gemeente.";
            }
        }
    }
    ...
    WHERE t_gemeenten.td_indexProvincies= $_provincieSelectie
    AND t_prijzen2020.d_prijzenV2020 != 0
    order by t_prijzen2020.d_prijzenV2020 ASC
    limit 1";
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) { 
            if ($_provincieSelectie == 2){ 
                $_formatPrijzen = number_format($row['d_prijzenV2020'], 0, '.','.'); 
                $_goedkoopsteGemeente="<p class='cijfer2'>".$_formatPrijzen."€</p><p class='tekstPijlen'>In het<br>Brussels Hoofdstedelijk Gewest <br>is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddelde prijs van<br>".$_formatPrijzen."€ de goedkoopste<br>gemeente.";
            }
            else
            {
                $_formatPrijzen = number_format($row['d_prijzenV2020'], 0, '.','.');
                $_goedkoopsteGemeente="<p class='cijfer2'>".$_formatPrijzen."€</p><p class='tekstPijlen'>In provincie $_provincieNaam <br>is <strong>".$row[$_gemeente_taal]."</strong> met een<br>gemiddelde prijs van<br>".$_formatPrijzen."€ de goedkoopste<br>gemeente."; 
            }
        }
    }
