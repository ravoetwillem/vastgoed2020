...
//Gemiddelde percentage 1 jaar in de provincie $_NAAM
  $_sql = "SELECT ROUND((((AVG(t_prijzen2020.d_prijzenV2020)-AVG(t_prijzen2019.d_prijzenV2019))/AVG(t_prijzen2019.d_prijzenV2019))*100),2) AS avgProvincie1j 
  FROM t_prijzen2020 
  ...
  $_sql = "SELECT ROUND((((AVG(t_prijzen2020.d_prijzenV2020)-AVG(t_prijzen2011.d_prijzenV2011))/AVG(t_prijzen2011.d_prijzenV2011))*100),2) AS avgProvincie10j 
  FROM t_prijzen2020 
...
//Gemiddelde prijs in de provincie $_NAAM
    ...
    $_sql = "SELECT SUM($_transacties2020) AS sumTransacties 
     ...
     if ($_result->num_rows > 0) {
         while($row = $_result->fetch_assoc()) {  
            $_tNAAM = number_format($row["sumTransacties"], 0, '.','.');
         }
     }
